...
class RegistedMoreThanWeekUser(BasePermission):
    
    message = '가입 후 7일 이상 지난 사용자만 사용하실 수 있습니다.'
    
    def has_permission(self, request, view):
        return bool(request.user.is_admin or request.user and request.user.join_date < (timezone.now() - timedelta(days=7)))
